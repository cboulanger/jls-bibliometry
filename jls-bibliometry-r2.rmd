...
```{r}
library('data.table')
gender_cache_file <- "jls-first-names-gender.csv"

# associate the firstnames of authors with data downloaded (and cached) from genderize.io
if (!file.exists(gender_cache_file)) {
  library(genderizeR)
  run_query('match (a:Author)-[:CREATOR_OF]->(w:Work)-[:PUBLISHED_IN]->(v:Venue)
WHERE (v.id = "j law soc" OR v.id = "br j law soc") and a.given <> ""
with apoc.text.split(apoc.text.replace(a.given,"[. ]",""), " ")[0] as name
where size(name) >= 3
return distinct name') |>
    findGivenNames() |>
    write.csv(file = gender_cache_file)
}
gender_table <- read.csv(file = gender_cache_file) |>
  select(gender, probability) |>
  rename(name = probability) |>
  as.data.table() |>
  setkey("name") |>
  select(gender, name)

# create table of male/female authors per year
run_query('match (a:Author)-[:CREATOR_OF]->(w:Work)-[:PUBLISHED_IN]->(v:Venue)
    WHERE (v.id = "j law soc" OR v.id = "br j law soc") and a.given <> "" and w.id starts with "10."
    return a.given as name, w.year as year, count(*) as count') |>
  as.data.table() |>
  setkey("name") |>
  merge(gender_table, by = "name") |>
  select(gender, year, count) -> dt
...
gender_by_year <- dt %>%
  ...
  ungroup() %>%
  mutate(sum = rowSums(.[, c("male", "female")])) %>%
  mutate(female_percent = female / sum * max(sum))
