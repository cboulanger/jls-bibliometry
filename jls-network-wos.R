...
journal_network <- read.csv('data/jls-journal-network-wos2-may-23.csv')
journal_network <- journal_network |> select(ref_source_title, source_title, ref_pubyear)
write.csv(journal_network, file = 'data/jls-journal-network-wos2-may-23.csv', row.names = FALSE)
...
if (clustering) {
  ...
  # inner_join(select(edges, from, count), by=c("id" = "from")) |>
  # group_by(label, id) |>
  # summarise(count = sum(count)) |>
  # ungroup() |>
  # rename(value=count)   
...
vn |>
  ...
  visSave('docs/jls-journal-network-wos2-may-23.html', selfcontained = T)
